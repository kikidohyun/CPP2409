...
void game_excution(User &user_A,vector<vector<int>> &map,int num,int check,int job);
...
int main() {
	...
	{ // 사용자에게 계속 입력받기 위해 무한 루프
		string user_input = ""; // 사용자의 입력을 저장할 변수
			...
			{
				// 위로 한 칸 올라가기
				game_excution(user_A,user_B, map, 8,job);
			}
			else if (user_input == "down")
			{
				// TODO: 아래로 한 칸 내려가기
				game_excution(user_A, user_B,map, 2,job);
			}
			else if (user_input == "left")
			{
				// TODO: 왼쪽으로 이동하기
				game_excution(user_A,user_B, map, 4,job);
			}
			else if (user_input == "right")
			{
				// TODO: 오른쪽으로 이동하기
				game_excution(user_A,user_B, map, 6,job);
			}
